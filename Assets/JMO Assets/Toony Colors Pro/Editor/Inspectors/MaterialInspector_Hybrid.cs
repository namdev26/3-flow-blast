	...
	namespace ShaderGenerator
	{
		public class MaterialInspector_Hybrid : ShaderGUI
		{
			...
			public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
			{
				...
				for (int i = 0; i < properties.Length; i++)
				{
					if (TCP2_MaterialPropertyUtility.GetPropertyType(properties[i]) == ShaderPropertyType.Float)
					{
						EditorGUIUtility.labelWidth = labelWidth - 50;
					}
					...
					if (ShowNextProperty)
					{
						...
						{
							if ((TCP2_MaterialPropertyUtility.GetPropertyFlags(properties[i]) & (ShaderPropertyFlags.HideInInspector | ShaderPropertyFlags.PerRendererData)) == ShaderPropertyFlags.None)
							{
